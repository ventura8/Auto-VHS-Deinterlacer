...
2. Hardware settings are detected from RAM, CPU, and GPU data.
3. A VapourSynth script is generated for the current source file.
4. `vspipe` or the native fallback streams frames into FFmpeg.
5. FFmpeg produces the final encoded output and the pipeline cleans temporary files.
