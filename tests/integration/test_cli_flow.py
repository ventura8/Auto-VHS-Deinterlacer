"""Integration tests for CLI input, cleanup, and utility fallback paths."""
...
def test_hardware_detection_various(ad):
    ...
    with patch("shutil.which", return_value="nvidia-smi"):
        with patch("subprocess.check_output", side_effect=OSError("SMI Error")):
            ad.detect_hardware_settings()
...
def test_log_vspipe_output_branches(ad):
    ...
    with patch("auto_deinterlancer.log_debug"):
        with patch("modules.runtime.vspipe.log_error") as mock_err:
...
def test_cleanup_on_exit_logic(ad):
    ...
    p_error.terminate.side_effect = OSError("Death")
    p_error.pid = 456

    with patch("modules.core.utils.ACTIVE_PROCS", [p_graceful, p_stuck, p_error]):
...
def test_utils_fallbacks(ad):
    ...
    with patch("subprocess.check_output", side_effect=OSError("Error")):
...
def test_main_exec(ad):
    ...
    with patch("shutil.which", return_value="/usr/bin/tool"):
        with patch("modules.runtime.pipeline.get_input_files", return_value=[Path("v.mp4")]):
            with patch("modules.runtime.pipeline.process_video") as mock_proc:
...
def test_parse_ffmpeg_time_error(ad):
    ...
    assert utils.parse_ffmpeg_time("time=00:XX:00") == (None, None, None)
