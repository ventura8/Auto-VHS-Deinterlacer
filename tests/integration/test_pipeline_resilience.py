"""Resilience integration tests for fallback paths and error handling."""
...
def test_process_video_sync_trim(ad):
    ...
    with patch("modules.runtime.pipeline.AUDIO_OFFSET", -5.0):
                ...
                with patch("modules.runtime.pipeline.get_duration", return_value=10.0):
                    with patch("modules.runtime.pipeline.cleanup_temp_files"):
                        with patch("modules.runtime.pipeline.shutil.which", return_value="/bin/tool"):
                            with patch("modules.runtime.pipeline.get_vpy_info", return_value=(1000, 30.0, 720, 576, "YUV420P10")):
                                with patch("modules.runtime.pipeline._run_encoding_pipeline", return_value=True) as mock_run:
                                    with patch("modules.runtime.pipeline.AUDIO_CODEC", "aac"):
                                        with patch("modules.runtime.pipeline.AUDIO_BITRATE", "320k"):
                                            with patch("os.path.exists", return_value=True):
                                                with (
                                                    patch("modules.runtime.pipeline.log_info"),
                                                    patch("modules.runtime.pipeline.log_debug"),
                                                    patch("modules.runtime.pipeline.log_error"),
                                                ):
    ...
    with patch.object(Path, "exists") as mock_exists:
        ...
        with patch.object(Path, "stat", return_value=mock_stat):
            ...
            with patch("modules.runtime.pipeline.create_vpy_script"):
                with patch("auto_deinterlancer.shutil.which", return_value="/bin/tool"):
                    with patch("auto_deinterlancer.log_info"), patch("auto_deinterlancer.log_debug"):
                        with patch("os.path.exists", return_value=True):
                            with patch("modules.runtime.pipeline.get_duration", return_value=10.0):
                                with patch("modules.runtime.pipeline.get_vpy_info", return_value=(1000, 30.0, 720, 576, "YUV420P10")):
                                    with patch("modules.runtime.pipeline._run_encoding_pipeline", return_value=False) as mock_run:
...
def test_process_video_av1_cpu_mode(ad):
    ...
    with (
        patch("auto_deinterlancer.HW_SETTINGS", {"use_gpu_opencl": False, "cpu_threads": 4}),
        patch("auto_deinterlancer.ENCODER", "av1"),
        patch.object(Path, "exists") as mock_exists,
        patch.object(Path, "stat", return_value=mock_stat),
        patch("auto_deinterlancer.get_duration", return_value=10.0),
        patch("auto_deinterlancer.get_start_time", return_value=0.0),
        patch("auto_deinterlancer.cleanup_temp_files"),
        patch("auto_deinterlancer.shutil.which", return_value="/bin/tool"),
        patch("auto_deinterlancer.log_info"),
        patch("auto_deinterlancer.log_debug"),
        patch("auto_deinterlancer.log_error"),
    ):
...
def test_main_no_files(ad):
    ...
    with patch("builtins.input", return_value=""):
        ...
        with patch("modules.runtime.pipeline.check_requirements"):
            ...
            with patch("modules.runtime.pipeline.get_input_files", return_value=[]):
...
def test_check_requirements_missing_tools(ad):
    ...
    with patch("shutil.which", side_effect=lambda x: None if x == "ffmpeg" else "/bin/" + x):
        with patch("modules.core.utils.log_error") as mock_log:
...
def test_detect_hardware_settings_windows_path(ad):
    ...
    mock_ctypes.Structure = type("_MockStructure", (), {"_fields_": []})

    for attr_name in ("c_ulong", "c_ulonglong"):
        setattr(mock_ctypes, attr_name, int)
    mock_ctypes.sizeof.return_value = 128
    mock_ctypes.byref.side_effect = str
...
def test_setup_environment_paths(ad):
    ...
    def _exists_paths(path):
        path_text = str(path).replace("\\", "/")
        if path_text.endswith("/.venv"):
            return False
        if path_text.endswith("/vs/plugins"):
            return False
        if path_text.endswith("/vs/vs-plugins"):
            return True
        return True

    with patch("os.path.exists", side_effect=_exists_paths):
                        ...
                        with patch("os.path.getsize", return_value=100):
                            with patch("auto_deinterlancer.log_debug"):
                                ...
                                written_content = "".join(
                                    call[0][0].decode() if isinstance(call[0][0], bytes) else call[0][0]
                                    for call in m_open().write.call_args_list
                                )
...
def test_create_vpy_auto_fps_ntsc(ad):
    ...
    with patch("auto_deinterlancer.TV_STANDARD", "auto"):
        with patch("modules.runtime.vspipe.get_fps", return_value=29.97):
            with patch("auto_deinterlancer.HW_SETTINGS", {"cpu_threads": 4, "ram_cache_mb": 4000, "use_gpu_opencl": False}):
...
def test_process_video_exception_handling(ad):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def exists_side_effect_fail(_path=None):
            return True

        with patch("auto_deinterlancer.Path.exists", side_effect=exists_side_effect_fail):
            with patch("modules.runtime.pipeline.create_vpy_script", side_effect=OSError("Test Error")):
                with patch("auto_deinterlancer.shutil.which", return_value="/bin/tool"):
                    with patch("modules.runtime.pipeline.log_error") as mock_log:
                        with patch("auto_deinterlancer.log_info"), patch("auto_deinterlancer.log_debug"):
...
def test_process_video_exception_with_vpy_cleanup(ad):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def exists_side_effect(_path=None):
            # If path argument is provided (some mock calls might pass it)
            return True

        with patch("modules.runtime.pipeline.create_vpy_script", side_effect=OSError("Test Error")):
            with patch("auto_deinterlancer.shutil.which", return_value="/bin/tool"):
                with patch("auto_deinterlancer.log_error"), patch("auto_deinterlancer.log_info"), patch("auto_deinterlancer.log_debug"):
...
def testget_input_files_with_deinterlaced_filter(ad):
    ...
    clean_file = MagicMock(spec=Path)
    clean_file.is_file.return_value = True
    clean_file.suffix = ".mp4"
    clean_file.name = "capture.mp4"

    filtered_file = MagicMock(spec=Path)
    filtered_file.is_file.return_value = True
    filtered_file.suffix = ".mp4"
    filtered_file.name = "capture_deinterlaced.mp4"
        ...
        with patch("builtins.input", return_value=""):
            with patch("modules.runtime.pipeline.Path.exists", return_value=True):
                with patch("modules.runtime.pipeline.Path.is_dir", return_value=True):
                    with patch("modules.runtime.pipeline.Path.iterdir", return_value=[clean_file, filtered_file]):
                        files = ad.get_input_files()
...
def test_detect_hardware_gpu_exception(ad):
    ...
    with patch("shutil.which", return_value="/usr/bin/nvidia-smi"):
        with patch("subprocess.check_output", side_effect=OSError("nvidia-smi failed")):
            with patch("auto_deinterlancer.PERF_PROFILE", "auto"):
                settings = ad.detect_hardware_settings()
