...
# Use deferred imports to avoid top-level execution of modules.core.config hardware detection
...
def test_utils_project_root_frozen():
    ...
    # We need to unimport modules.core.utils if it's already imported?
...
def test_vspipe_log_errors():
    ...
    from modules.runtime import vspipe
    ...
    with patch("subprocess.Popen") as mock_popen:
        ...
        with patch("modules.runtime.pipeline.get_vspipe_env", return_value={}):
            with patch("threading.Thread"):
                result = pipeline._run_encoding_pipeline(vspipe_cmd, ffmpeg_cmd, Path("t.vpy"), 100)
...
def test_vspipe_parse_info_malformed():
    ...
    # Format Name: empty value should parse as an empty string.
    t, f, w, h, fmt = vspipe._parse_vspipe_info_output("Format Name:")
    assert fmt == ""
