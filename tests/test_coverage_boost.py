...
def test_get_input_files_full(ad):
    """Cover the interactive prompt logic and folder scanning in _get_input_files."""
...
def test_log_vspipe_output_branches(ad):
    ...
    with patch('auto_deinterlancer.log_debug'):
        with patch('auto_deinterlancer.log_error') as mock_err:
...
def test_cleanup_on_exit_logic(ad):
    ...
    with patch('auto_deinterlancer.ACTIVE_PROCS', [p_graceful, p_stuck, p_error]):
...
def test_main_exec(ad):
    ...
    with patch('shutil.which', return_value='/usr/bin/tool'):
        with patch('auto_deinterlancer._get_input_files', return_value=[Path("v.mp4")]):
            with patch('auto_deinterlancer.process_video') as mock_proc:
