...
try {
    ...
    Invoke-Step "Generate Coverage Summary" {
        ...
        # The report contains non-ASCII characters. PowerShell decodes a native
        # command's stdout using [Console]::OutputEncoding, which is the OEM code
        # page on Windows and would mangle them before they reach the file.
        ...
        # Write LF line endings: WriteAllLines would use CRLF, which leaves the
        # tracked file failing the mdformat check after every run on Windows.
        $summaryText = (([string[]]$summary) -join "`n") + "`n"
