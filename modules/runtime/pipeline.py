...
from modules.runtime.vspipe import create_vpy_script, get_vpy_info, log_vspipe_output
...
def _scan_directory(path: Path, video_exts: set) -> list:
    ...
    return [
        ...
        if (f.is_file() and f.suffix.lower() in video_exts and "_deinterlaced" not in f.name and "_intermediate" not in f.name)
    ]
...
def _calculate_audio_sync(input_path: Path, video_duration: float) -> float:
    ...
    if abs_diff > min_drift:
        if diff < 0:
            ...
            log_info("   [SYNC] Video duration is zero. Ignoring safely.")
        else:
...
def _run_encoding_pipeline(vspipe_cmd, ffmpeg_cmd, temp_script, duration_sec):
    ...
    try:
        ...
        if p_ffmpeg.returncode == 0:
            # FFmpeg often reports final "time=" slightly below stream duration,
            # so force a terminal 100% progress update on successful completion.
            if total_ts:
...
def process_video(input_path: Path):
    ...
    def _format_speed(speed_x: float) -> str:
        if speed_x is None or speed_x <= 0:
            return "N/A"
        return f"{speed_x:.2f}x"

    def _log_video_summary(status: str, output_path: Path = None, speed_x: float = None):
    ...
    if output_file.exists():
        ...
        if existing_duration > 0:
            ...
            cleanup_temp_files(work_dir, stem)
            _log_video_summary("skipped", output_file)
            return {
                "input": input_path,
                "output": output_file,
                "status": "skipped",
    ...
    cleanup_temp_files(work_dir, stem)
    elapsed_sec = time.time() - started_at
    speed_x = (duration_sec / elapsed_sec) if elapsed_sec > 0 and duration_sec > 0 else None
    final_status = "success" if success else "failed"
    _log_video_summary(final_status, output_file, speed_x)
    return {
        "input": input_path,
        "output": output_file,
        "status": final_status,
...
def main():
    ...
    for i, f in enumerate(input_files):
        ...
        try:
            ...
            log_error(f"[ERROR] Failed processing {f}: {error}")
            continue
        if result:
    ...
    if len(input_files) > 1:
        ...
        failed_count = sum(1 for r in results if r.get("status") in {"failed", "not_found"})
        speed_rows = [
            r for r in results if r.get("status") == "success" and (r.get("duration_sec") or 0) > 0 and (r.get("elapsed_sec") or 0) > 0
        ]
        total_src_sec = sum(float(r.get("duration_sec") or 0.0) for r in speed_rows)
        total_elapsed_sec = sum(float(r.get("elapsed_sec") or 0.0) for r in speed_rows)
        batch_speed = (total_src_sec / total_elapsed_sec) if total_elapsed_sec > 0 else None
        ...
        log_info(f"   Speed   : {f'{batch_speed:.2f}x' if batch_speed and batch_speed > 0 else 'N/A'}")
        log_info("   Videos  :")
        for r in results:
            input_name = r.get("input").name if r.get("input") else "unknown"
            output_obj = r.get("output")
            output_name = output_obj.name if output_obj else "-"
            status_text = str(r.get("status", "unknown")).upper()
            speed_x = r.get("speed_x")
            speed_text = f"{float(speed_x):.2f}x" if speed_x and float(speed_x) > 0 else "N/A"
            log_info(f"     - {input_name} -> {status_text} (speed: {speed_text}, output: {output_name})")
